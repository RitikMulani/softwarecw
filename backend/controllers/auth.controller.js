 ...
 * @returns {Object} { accessToken, refreshToken } both valid for their respective durations
 */
 ...
 * Validates input, checks for duplicates, hashes password, and stores in database
 */
export async function register(req, res) {
  try {
    ...
    res.status(201).json({
      ...
      userId: result[0]?.id || result?.id || 'registered',
    });
  } catch (error) {
  ...
  try {
    ...
    if (user.user_type === 'patient') {
      const [patients] = await db.query(
        'SELECT blood_group, allergies, chronic_conditions, emergency_contact, emergency_phone FROM patients WHERE user_id = $1',
        [user.id]
      );
      ...
      const [doctors] = await db.query(
        'SELECT specialization, qualification, experience_years, license_number, consultation_fee, available_days, available_hours FROM doctors WHERE user_id = $1',
        [user.id]
      );
...
export async function updateProfile(req, res) {
  try {
    ...
    // Get user type first
    const [users] = await db.query('SELECT user_type FROM users WHERE id = $1', [userId]);
